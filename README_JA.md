...
### 19. カスタマイズ可能なプロジェクトパス

オリジナル版はディスク上のレイアウト（`raw/`、`wiki/`、`purpose.md` など）がハードコードされています。本バージョンでは、オプションの YAML ファイルで上書き可能にしました：

- **2 階層設定** — プロジェクトレベル `<project>/.llm-wiki/paths.yaml`（最優先）とグローバル `<app_data_dir>/paths.yaml`（プロジェクト横断のデフォルト）
- **3 階層フォールバック** — プロジェクト → グローバル → 組み込みデフォルト、フィールド単位のマージ
- **15 個の上書き可能フィールド** — すべてのレイアウトディレクトリとトップレベルファイル
- **設定 UI** — `Settings → Project paths` パネルにライブフォーム、保存、「デフォルトに戻す」ボタンを搭載
- **アトミック書き込み** — ディスク上に書き込み途中の yaml ファイルが残ることはない（`tmp` + rename）
- **バリデーション** — `open_project` 時に `..` のトラバーサルを拒否。絶対パスは Task 15.1 緩和で許可され、そのまま保存されます(プロジェクト外に wiki を置ける)が、yaml はマシン間で移植できなくなります。
- **スキーマバージョン** — 互換性のない `version:` を持つファイルは拒否。プロジェクトレベルファイルは厳格、グローバルファイルは寛容（ログ出力のみで無視）
- **マイグレーション不要** — `paths.yaml` のない既存プロジェクトはそのまま開ける

エンドユーザー向けドキュメントは [`docs/user/paths-yaml.md`](docs/user/paths-yaml.md) を参照。設計根拠は [`docs/plans/2026-06-01-configurable-paths.md`](docs/plans/2026-06-01-configurable-paths.md) を参照。
